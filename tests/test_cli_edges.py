...
def test_cli_global_flags_and_clean(tmp_path, monkeypatch):
    ...
    r = runner.invoke(
        app, ["--data-dir", str(data_dir), "--log-file", str(log_file), "--log-level", "DEBUG", "run", "--stage", "all"]
    )
    ...
    # clean sem prompt
    r2 = runner.invoke(app, ["--data-dir", str(data_dir), "--log-file", str(log_file), "clean", "--yes"])
...
def test_cli_config_alt_envfile(tmp_path):
    ...
    env_alt.write_text(f'DATA_DIR="{data_dir}"\nLOG_FILE="{log_file}"\nLOG_LEVEL=DEBUG\n', encoding="utf-8")
